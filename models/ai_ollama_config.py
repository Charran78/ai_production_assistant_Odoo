...
class AIOllamaConfig(models.Model):
    ...
    url = fields.Char(
        string="URL de Ollama", default="http://localhost:11434", required=True
    )
    timeout = fields.Integer(string="Timeout (segundos)", default=120)

    # Parámetros de Rendimiento
    num_ctx = fields.Integer(
        string="Ventana de Contexto",
        default=2048,
        help=(
            "Tamaño de la memoria de conversación. Reducir a 1024 o 512 mejora "
            "drásticamente la velocidad en hardware modesto."
        ),
    )
    temperature = fields.Float(
        string="Creatividad (Temperatura)",
        default=0.7,
        ...
        try:
            ...
            return {
                ...
                "params": {
                    ...
                    "message": self.env._(
                        "Conexión establecida con Ollama en %s"
                    )
                    % self.url,
                    "type": "success",
                    "sticky": False,
            ...
            raise ValidationError(
                self.env._("Error conectando a Ollama: %s") % str(e)
            ) from e
