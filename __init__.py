...
def _create_default_data(cr, registry):
    ...
    try:
        ...
        if hasattr(env['ai.ollama.model'], 'get_default_models'):
            ...
            _logger.warning("AI Production Assistant: Método get_default_models no encontrado")
        ...
        cron = cron_model.search([('id', '=', env.ref('ai_production_assistant.ir_cron_process_ai_queue', raise_if_not_found=False))])
        if not cron:
            cron_model.create({
                'name': 'Procesar cola de IA',
                'model_id': env.ref('ai_production_assistant.model_ai_assistant_message').id,
                'state': 'code',
