...
try:
    ...
    GPUtil = None
    _logger.warning("GPUtil no está instalado. La información de la GPU no estará disponible.")
...
class InstallationWizard(models.TransientModel):
    ...
    ], string='Estado de Ollama', readonly=True)
    
    # Paso 2: Modelos disponibles
    available_models = fields.Text(string='Modelos Disponibles', readonly=True)
    selected_model = fields.Selection(selection='_get_dynamic_model_options', string='Modelo Instalado', default=None)
    ...
    def _get_dynamic_model_options(self):
        ...
        options = []
        _logger.info("Iniciando _get_dynamic_model_options para obtener opciones de modelos.")
        
        # Obtener información de hardware para recomendaciones
        cpu_count = self.cpu_count
        ram_total = self.ram_total
        gpu_info = self.gpu_info # Esto ya es un JSON string o 'No GPU detected'
        ...
        if current_ollama_status == 'running':
            _logger.info("Ollama está ejecutándose. Obteniendo modelos instalados localmente.")
            # Obtener modelos instalados localmente
            installed_models_data = self._get_available_models()
            _logger.info(f"Modelos instalados localmente encontrados: {len(installed_models_data.get('models', []))}")
            for model in installed_models_data.get('models', []):
                try:
                    ...
                    if model_name and parameter_size:
                        options.append((model_name, f"{model_name} (Instalado - {parameter_size}){recommendation_text}"))
                        _logger.info(f"Añadido modelo instalado: {model_name}")
                    else:
                        current_log += f'⚠️ Modelo instalado con estructura inesperada: {model_name} - {model_details}\n'
                        _logger.warning(f'Modelo instalado con estructura inesperada: {model_name} - {model_details}')
                except Exception as e:
                    current_log += f'❌ Error procesando modelo instalado {model.get("name", "desconocido")}: {str(e)}\n'
                    _logger.error(f'Error procesando modelo instalado {model.get("name", "desconocido")}: {str(e)}')
            
            _logger.info("Obteniendo modelos remotos.")
            # Obtener modelos remotos
            remote_models_data = self._get_remote_models()
            _logger.info(f"Modelos remotos encontrados: {len(remote_models_data.get('models', []))}")
            for model in remote_models_data.get('models', []):
                try:
                    ...
                    if model_name and parameter_size and model_name not in [opt[0] for opt in options]:
                        options.append((f"remote_{model_name}", f"[Remoto] {model_name} ({parameter_size}){recommendation_text}"))
                        _logger.info(f"Añadido modelo remoto: {model_name}")
                    elif model_name and parameter_size:
                        current_log += f'ℹ️ Modelo remoto {model_name} ya está instalado localmente, no se añade como opción remota.\n'
                        _logger.info(f'Modelo remoto {model_name} ya está instalado localmente, no se añade como opción remota.')
                    else:
                        current_log += f'⚠️ Modelo remoto con estructura inesperada: {model_name} - {model_details}\n'
                        _logger.warning(f'Modelo remoto con estructura inesperada: {model_name} - {model_details}')
                except Exception as e:
                    current_log += f'❌ Error procesando modelo remoto {model.get("name", "desconocido")}: {str(e)}\n'
                    _logger.error(f'Error procesando modelo remoto {model.get("name", "desconocido")}: {str(e)}')
            
            self.write({'installation_log': current_log}) # Guardar log
            _logger.info(f"Total de opciones de modelos generadas: {len(options)}")
            if options:
                return options
        
        current_log += 'ℹ️ No hay modelos disponibles (Ollama no está ejecutándose o no hay conexión).\n'
        self.write({'installation_log': current_log}) # Guardar log
        _logger.info("No hay modelos disponibles (Ollama no está ejecutándose o no hay conexión).")
    ...
    vector_db_status = fields.Selection([
        ...
        ('ready', 'Listo')
    ], string='Estado de Vector DB', readonly=True)
    
    # Paso 4: Configuración Avanzada (Proactividad)
    enable_watchdogs = fields.Boolean(string='Activar Watchdogs (Alertas Proactivas)', default=True)
    enable_dashboards = fields.Boolean(string='Activar Dashboards Ejecutivos', default=True)
    enable_actions = fields.Boolean(string='Permitir Ejecución de Acciones Automáticas', default=False)
    show_pending_actions = fields.Boolean(string='Mostrar Acciones Pendientes en Dashboard', default=True)
    advanced_config_saved = fields.Boolean(string='Configuración Avanzada Guardada', readonly=True)
    ...
    ], string='Paso Actual', default='detection')
    ...
    def default_get(self, fields_list):
        res = super(InstallationWizard, self).default_get(fields_list)
        ...
        if ollama_status['ollama_status'] == 'running':
            ...
            res['available_models'] = json.dumps(models, indent=2)
    ...
    def _detect_ollama(self):
        ...
        try:
            ...
            process = subprocess.run(['ollama', '--version'], 
    ...
    def _get_available_models(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # Registrar modelos detectados en el log para depuración
                current_log += f'🔍 Modelos detectados: {[model["name"] for model in models_data.get("models", [])]}\n'
                self.write({'installation_log': current_log}) # Guardar log
                return models_data
            else:
                current_log += f'❌ Error en API de Ollama: Código de estado {response.status_code}\n'
                self.write({'installation_log': current_log}) # Guardar log
    ...
    def _get_remote_models(self):
        ...
        current_log += f'🌐 Cargando lista de {len(curated_models)} modelos recomendados...\n'
    ...
    def _get_system_info(self):
        ...
        try:
            ...
            current_log += f'💻 CPU: {system_info["cpu_count"]} cores, {system_info["cpu_freq"]:.2f} MHz\n'

            # RAM Info
            svmem = psutil.virtual_memory()
            system_info['ram_total'] = round(svmem.total / (1024**3), 2) # GB
            current_log += f'💾 RAM Total: {system_info["ram_total"]:.2f} GB\n'
                ...
                try:
                    ...
                    if gpus:
                        ...
                        for gpu in gpus:
                            ...
                            current_log += f'🎮 GPU {gpu.id}: {gpu.name} ({gpu.memoryTotal}MB, {gpu.load*100:.1f}% util)\n'
                        system_info['gpu_info'] = json.dumps(gpu_info_list)
                    else:
                        system_info['gpu_info'] = 'No GPU detected'
                        current_log += '🎮 GPU: No GPU detectada\n'
                except Exception as gpu_e:
                    system_info['gpu_info'] = f'Error getting GPU info: {str(gpu_e)}'
                    current_log += f'❌ Error obteniendo información de GPU: {str(gpu_e)}\n'
                    _logger.warning(f"GPUtil error: {gpu_e}. It might not be installed or no GPUs are present.")
            else:
                system_info['gpu_info'] = 'GPUtil no está disponible, información de GPU no detectada.'
    ...
    def action_install_ollama(self):
        ...
        try:
            ...
            if system == 'windows':
                ...
                try:
                    process = subprocess.run(['curl', '-fsSL', 'https://ollama.ai/install.sh'], 
                                           capture_output=True, text=True, timeout=30)
                    if process.returncode == 0:
                        install_script = process.stdout
                        # Ejecutar script de instalación
                        install_process = subprocess.run(['sh', '-c', install_script], 
                                                       capture_output=True, text=True, timeout=120)
                        current_log += install_process.stdout
                        if install_process.returncode == 0:
                            current_log += '✅ Ollama instalado correctamente en Linux\n'
                        else:
                            current_log += f'❌ Error en instalación: {install_process.stderr}\n'
    ...
    def action_download_model(self):
        ...
        if not self.selected_model:
            current_log += '❌ Error: No se ha seleccionado ningún modelo para descargar.\n'
            self.write({'installation_log': current_log})
            raise UserError(_('Por favor, selecciona un modelo para descargar.'))
        ...
        if model_to_download.startswith('remote_'):
            ...
            current_log += f'📥 Verificando/actualizando modelo local {model_to_download}...\n'
        
        try:
            process = subprocess.run(['ollama', 'pull', model_to_download], 
            ...
            models = self._get_available_models()
            self.write({'available_models': json.dumps(models, indent=2), 'installation_log': current_log}) # Guardar log y modelos
    ...
    def action_start_ollama(self):
        ...
        try:
            ...
            if system == 'windows':
                ...
                process = subprocess.run(['ollama', 'serve'], 
                                      capture_output=True, text=True, timeout=10)
                current_log += process.stdout
                
            elif system in ['linux', 'darwin']:
                process = subprocess.run(['systemctl', 'start', 'ollama'], 
                                      capture_output=True, text=True, timeout=30)
                current_log += process.stdout
            
            # Verificar estado
            new_status = self._detect_ollama()
            self.write({**new_status, 'installation_log': current_log}) # Guardar log y nuevo estado
    ...
    def action_configure_database(self):
        ...
        try:
            ...
            self.write({
                ...
                'installation_log': current_log + '✅ Base de datos vectorial configurada\n'
            })
    ...
    def action_save_advanced_config(self):
        ...
        self.write({
            ...
            'installation_log': current_log + '✅ Configuración avanzada guardada\n'
        })
        return self._show_installation_view()
        ...
        try:
            if self.current_step == 'detection':
                ...
                if self.ollama_status != 'running':
                    raise UserError(_('Ollama debe estar ejecutándose para continuar'))
                ...
                if not self.selected_model:
                    raise UserError(_('Debes seleccionar un modelo para continuar'))
                ...
                if self.vector_db_status not in ['configured', 'ready']:
                    raise UserError(_('La base de datos debe estar configurada para continuar'))
            ...
            self.write({'installation_log': current_log}) # Guardar log de error
            raise UserError(_('Error avanzando al siguiente paso: %s') % str(e))
    ...
    def action_previous_step(self):
        ...
        try:
            ...
            self.write({'installation_log': current_log}) # Guardar log de error
            raise UserError(_('Error retrocediendo al paso anterior: %s') % str(e))
    ...
    def action_complete_installation(self):
        ...
        try:
            ...
            if self.ollama_status != 'running':
                raise UserError(_('Ollama debe estar ejecutándose para completar la instalación'))
            ...
            if self.selected_model:
                ...
                # Buscar si ya existe
                ollama_model = self.env['ai.ollama.model'].search([('name', '=', model_name)], limit=1)
                if not ollama_model:
                ...
                self.env['ir.config_parameter'].sudo().set_param('ai_production_assistant.selected_model', model_name)
                current_log += f'✅ Modelo {model_name} establecido como predeterminado\n'

            # 4. Guardar configuración avanzada
            set_param = self.env['ir.config_parameter'].sudo().set_param
            set_param('ai_production_assistant.enable_watchdogs', str(self.enable_watchdogs))
            set_param('ai_production_assistant.enable_dashboards', str(self.enable_dashboards))
            set_param('ai_production_assistant.enable_actions', str(self.enable_actions))
            set_param('ai_production_assistant.show_pending_actions', str(self.show_pending_actions))
            ...
            self.write({'installation_log': current_log}) # Guardar log de error
            raise UserError(_('Error completando instalación: %s') % str(e))
    ...
    def action_installation_wizard(self):
        ...
        """
        return {
            'name': _('Asistente de Instalación de AI Production Assistant'),
            'type': 'ir.actions.act_window',
            'res_model': 'ai.installation.wizard',
            'view_mode': 'form',
            'view_id': self.env.ref('ai_production_assistant.view_installation_wizard_form').id,
            'target': 'new',
